...
def generate_heatmap(indices_list, indices_name_mapping, frequencies, last_bd):
    ...
    indices_heatmap = hv.Overlay(heatmaps).opts(opts.Overlay(show_legend=False, height=200))
...
def generate_averages_chart(indices_list, indices_name_mapping, frequencies, last_bd, widget_name):
    ...
    for index in indices_list:
        ...
        start_date_1Y = pd.to_datetime(start_date_1Y)
        filtered_prices_1Y = historical_prices.loc[pd.to_datetime(historical_prices['date']).dt.tz_localize(None) >= pd.to_datetime(start_date_1Y)]
        start_date_10Y = (last_bd - frequencies['10Y'] * us_bd).to_pydatetime().date()
        start_date_10Y = pd.to_datetime(start_date_10Y)
        filtered_prices_10Y = historical_prices.loc[pd.to_datetime(historical_prices['date']).dt.tz_localize(None) >= pd.to_datetime(start_date_10Y)]
    ...
    def load_index_averages_graph(index_name):
        ...
        avg_graph = hv.Overlay(avg_graph_ticker_line * avg_graph_1Y_avg_line * avg_graph_10Y_avg_line * avg_graph_hist_avg_line).opts(legend_position='top', title = index_name + ': Moving Averages and RSI')
