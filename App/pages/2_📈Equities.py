...
st.set_page_config(page_title="Equities", page_icon="📈")

#Title of the Application
st.title("Equities")

#Sub-title of the Application
st.write("This section presents an overview of global equities performance, including Regions and Countries")

# Existing Streamlit page content
st.sidebar.header("Equities Sections")

#********** Equity Regions Section **********
st.header("Equity - Regions")
st.sidebar.markdown('<a href="#equity-regions" style="color: black;">- Equity - Regions</a>', unsafe_allow_html=True)

#Define the List of Equity Regions Indices
world_index = 'EUNL.DE'
us_index = 'SXR4.DE'
european_index = 'EUNK.DE'
japan_index = 'SXR6.DE'
apac_ex_japan_index = 'SXR1.DE'
em_index = 'QDVS.DE'
asia_em_index = 'CEBL.DE'
latam_em_index = 'LTAM.AS'

#Create an object with the list of objects
equity_region_indices = [world_index, us_index, european_index, japan_index, apac_ex_japan_index, em_index, asia_em_index, latam_em_index]

#Name Mapping 
equity_region_index_name_mapping = {'EUNL.DE': 'MSCI World', 'SXR4.DE':'MSCI USA', 'EUNK.DE':'MSCI Europe', 'SXR6.DE': 'MSCI Japan',
                                     'SXR1.DE': 'MSCI APAC ex-Japan', 'QDVS.DE': 'MSCI Emerging Markets (EM)', 'CEBL.DE': 'MSCI EM Asia', 
                                     'LTAM.AS': 'MSCI EM Latin America'}

#Equity Regional Indices - Performance Table
st.subheader("Equity - Regions Performance Table (%)")
...
generate_heatmap(indices_list = equity_region_indices, indices_name_mapping = equity_region_index_name_mapping, frequencies = frequencies_table, last_bd = last_bd)

#Equity Regional Indices - Performance Chart
st.subheader("Equity - Regions Technical Analysis Chart (%)")

st.write("Note: This subsection uses ETFs for performance. A table with the ETF corresponding to each label is presented at the bottom of the page")

generate_technical_analysis_chart(indices_list = equity_region_indices, indices_name_mapping = equity_region_index_name_mapping, frequencies = frequencies_graph, last_bd = last_bd, widget_name = 'Select Region, Equity Regions')

#********** Equity Country Section **********
st.header("Equity - Countries")
st.sidebar.markdown('<a href="#equity-countries" style="color: black;">- Equity - Countries</a>', unsafe_allow_html=True)

#Define the List of Equity Regions Indices
sp500_index = 'SXR8.DE'
stoxx50_index = 'LYP6.DE'
ftse100_index = 'SXRW.DE'
switzerland_index = 'XSMI.DE'
japan_index = 'SXR6.DE'
australia_index = 'LYPU.DE'
china_index = '36BZ.DE'
india_index = 'QDV5.DE'
brazil_index = 'LBRA.DE'
korea_index = 'LKOR.DE'
mexico_index = 'D5BI.DE'
south_africa_index = 'IBC4.F'
turkey_index = 'LTUR.DE'
saudi_arabia_index = 'IUSS.DE'
poland_index = 'IBCJ.DE'

#Create an object with the list of objects
equity_country_indices = [sp500_index, stoxx50_index, ftse100_index, switzerland_index, japan_index, australia_index, china_index, india_index, brazil_index, korea_index, mexico_index, south_africa_index, turkey_index, saudi_arabia_index, poland_index]

#Name Mapping 
equity_country_index_name_mapping = {'SXR8.DE': 'S&P 500', 'LYP6.DE':'Euro Stoxx 500', 'SXRW.DE':'UK FTSE 100', 
                                     'XSMI.DE': 'Solactive Swiss', 'SXR6.DE': 'MSCI Japan', 'LYPU.DE': 'Australia S&P/ASX 200', 
                                     '36BZ.DE': 'MSCI China', 'QDV5.DE': 'MSCI India', 'LBRA.DE': 'MSCI Brazil', 'LKOR.DE': 'MSCI Korea',
                                     'D5BI.DE': 'MSCI Mexico', 'IBC4.F': 'MSCI South Africa', 'LTUR.DE': 'MSCI Turkey', 
                                     'IUSS.DE': 'MSCI Saudi Arabia', 'IBCJ.DE': 'MSCI Poland'}

#Equity Regional Indices - Performance Table
st.subheader("Equity - Countries Performance Table (%)")

st.write("Note: This subsection uses ETFs for performance. A table with the ETF corresponding to each label is presented at the bottom of the page")

generate_heatmap(indices_list = equity_country_indices, indices_name_mapping = equity_country_index_name_mapping, frequencies = frequencies_table, last_bd = last_bd)

#Equity Regional Indices - Performance Chart
st.subheader("Equity - Countries Technical Analysis Chart (%)")

st.write("Note: This subsection uses ETFs for performance. A table with the ETF corresponding to each label is presented at the bottom of the page")

generate_technical_analysis_chart(indices_list = equity_country_indices, indices_name_mapping = equity_country_index_name_mapping, frequencies = frequencies_graph, last_bd = last_bd, widget_name = 'Select Country, Equity Countries')

#Table with ETFs corresponding to each label
#Equity Regions - ETF Table
st.subheader("Equities - Countries - ETF Table")

equities_region_etf_table = """
| Label                      | ETF Name                                     | ETF Ticker |
|----------------------------|----------------------------------------------|------------|
| MSCI World                 | iShares Core MSCI World UCITS ETF            | EUNL       |
| MSCI USA                   | iShares MSCI EM Latin America UCITS ETF      | SXR4       |
| MSCI Europe                | iShares Core MSCI Europe UCITS ETF EUR       | EUNK       |
| MSCI Japan                 | iShares MSCI Japan SRI UCITS ETF             | SXR6       |
| MSCI APAC ex-Japan         | iShares Core MSCI Pacific ex-Japan UCITS ETF | SXR1       |
| MSCI Emerging Markets (EM) | iShares MSCI EM SRI UCITS ETF                | QDVS       |
| MSCI EM Asia               | iShares MSCI EM Asia UCITS ETF               | CEBL       |
| MSCI EM Latin America      | iShares MSCI EM Latin America UCITS ETF      | LTAM       |
"""
    
st.write(equities_region_etf_table)

#Equity Country - ETF Table
st.subheader(" ")
st.subheader("Equities - Countries - ETF Table")

equities_country_etf_table = """
| Label                 | ETF Name                                   | ETF Ticker |
|-----------------------|--------------------------------------------|------------|
| S&P 500               | iShares Core S&P 500 ETF USD               | SXR8       |
| Euro Stoxx 500        | Lyxor Core STOXX Europe 600                | LYP6       |
| UK FTSE 100           | iShares Core FTSE 100 UCITS ETF GBP        | SXRW       |
| Solactive Swiss       | Xtrackers Switzerland UCITS ETF            | XSMI       |
| MSCI Japan            | iShares MSCI Japan SRI UCITS ETF           | SXR6       |
| Australia S&P/ASX 200 | Lyxor Australia (S&P/ASX 200) UCITS ETF    | LYPU       |
| MSCI China            | iShares MSCI China A UCITS ETF             | 36BZ       |
| MSCI India            | iShares MSCI India UCITS ETF USD           | QDV5       |
| MSCI Brazil           | Lyxor MSCI Brazil UCITS ETF                | LBRA       |
| MSCI Korea            | Lyxor MSCI Korea ETF                       | LKOR       |
| MSCI Mexico           | iShares MSCI South Korea ETF               | D5BI       |
| MSCI South Africa     | Xtrackers MSCI Mexico UCITS ETF            | IBC4       |
| MSCI Turkey           | Lyxor MSCI Turkey UCITS ETF                | LTUR       |
| MSCI Saudi Arabia     | iShares MSCI Saudi Arabia Capped UCITS ETF | IUSS       |
| MSCI Poland           | iShares MSCI Poland UCITS ETF              | IBCJ       |
"""
    
st.write(equities_country_etf_table)