...
st.set_page_config(page_title="Asset Classes", page_icon="🌐")
    
#Title of the Application
st.title("Asset Classes")

#Sub-title of the Application
st.write("This page presents an overview of multiple asset classes performance")

# Existing Streamlit page content
st.sidebar.header("Asset Class Sections")

#********** Asset Class Section **********
st.header("Asset Classes")
st.sidebar.markdown('<a href="#asset-classes" style="color: black;">Asset Classes</a>', unsafe_allow_html=True)

#Define the List of Asset Class Indices
#List of Asset Class Indices
global_equities_index = 'IUSQ.DE'
global_agg_index = 'GLAG.MI'
global_reits_index = 'SPYJ.DE'
commodities_index = 'EXXY.DE'
gold_index = 'XAD5.MI'
eur_cash_index = 'IEGE.AS'

#Create an object with the list of objects and reverse the order of priority (as it will get then reversed in the heatmap)
asset_class_indices = [global_equities_index, global_agg_index, global_reits_index, commodities_index, gold_index, eur_cash_index]

#Name Mapping 
asset_class_index_name_mapping = {'IUSQ.DE':'Global Equities', 'GLAG.MI':'Global Bond Aggregate', 'SPYJ.DE': 'Global Real Estate',
                                  'EXXY.DE': 'Commodities', 'XAD5.MI': 'Gold', 
                                  'IEGE.AS': 'Euro Short-term Treasuries'} 

#Asset Class - Performance Table
st.subheader("Asset Class Performance Table (%)")
...
generate_heatmap(indices_list = asset_class_indices, indices_name_mapping = asset_class_index_name_mapping, frequencies = frequencies_table, last_bd = last_bd)
    
#Asset Class - Performance Chart
st.subheader("Asset Class Multi-Period Performance Chart (%)")

st.write("Note: This subsection uses ETFs for performance. A table with the ETF corresponding to each label is presented at the bottom of the page")

generate_multiperformance_chart(indices_list = asset_class_indices, indices_name_mapping = asset_class_index_name_mapping, frequencies = frequencies_graph, last_bd = last_bd, widget_name = 'Select Timeperiod, Asset Class')

#Table with ETFs corresponding to each label

st.subheader("Asset Classes - ETF Table")

asset_classes_etf_table = """
| Label                      | ETF Name                                        | ETF Ticker |
|----------------------------|-------------------------------------------------|------------|
| Global Equities            | iShares MSCI ACWI UCITS ETF                     | IUSQ       |
| Global Bond Aggregate      | SPDR® Bloomberg Global Aggregate Bond UCITS ETF | GLAG       |
| Global Real Estate         | SPDR® Dow Jones Global Real Estate UCITS ETF    | SPYJ       |
| Commodities                | iShares Diversified Commodity Swap UCITS ETF    | EXXY       |
| Gold                       | Xtrackers Physical Gold ETC                     | XAD5       |
| Euro Short-term Treasuries | iShares € Govt Bond 0-1yr UCITS ETF             | IEGE       |
"""
    
st.write(asset_classes_etf_table)